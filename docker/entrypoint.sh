...
# --- 7. Стартовые данные ---
# Пустая база бесполезна: каталог позиций, библиотека атрибутов и резюме —
# это то, ради чего приложение открывают. Команда сама пропускает работу,
# если позиции уже есть, поэтому безопасна при каждом рестарте и не трогает
# данные, накопленные пользователями.
#
# Сеется полный набор, тот же что и локально: демо-кандидаты с резюме,
# лайками и обсуждением. Это стенд для показа проекта, а не боевая база с
# чужими персональными данными, и пустые экраны на нём бесполезны.
#
