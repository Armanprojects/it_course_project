...
# --- 7. Стартовые данные ---
# Пустая база бесполезна: каталог позиций и библиотека атрибутов — это то,
# ради чего приложение открывают. Команда сама пропускает работу, если
# позиции уже есть, поэтому безопасна при каждом рестарте и не трогает
# данные, накопленные пользователями.
#
# --minimal: только атрибуты и три публичные позиции. Демо-кандидатов с
# их резюме на боевом стенде быть не должно — они попадут в поиск рекрутёра
# вперемешку с настоящими.
#
# Выключается переменной SEED_ON_START=0 в дашборде Render.
